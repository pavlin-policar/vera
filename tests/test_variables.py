...
from veca import preprocessing as pp


class TestExplanatoryVariable1(unittest.TestCase):
    ...
    def test_split_variable_by_region_composite_explanatory_variables(self):
        ...
        assert isinstance(expl_v, veca.variables.CompositeExplanatoryVariable)
        expl_v = v.explanatory_variables[0]
        split_parts = expl_v.split_region()
        self.assertEqual(
            len(split_parts), 2, "Region was split into incorrect number of parts."
...
class TestExplanatoryVariable(unittest.TestCase):
    def test_can_merge_with_discretized_variables(self):
        ...
        variables = veca.an.generate_explanatory_features(df, z, scale_factor=0.5)
